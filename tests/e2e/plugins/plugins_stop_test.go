...
var _ = ginkgo.Describe("Stop Plugin", func() {
	table.DescribeTable("create stop route plugin route and test",
		...
		table.Entry("test python runner stop plugin route success", tools.HttpTestCase{
			Object:       tools.GetA6Expect(),
			Method:       http.MethodGet,
			Path:         "/test/python/runner/stop",
			ExpectStatus: http.StatusCreated,
			ExpectHeaders: map[string]string{
				"X-Resp-A6-Runner": "Python",
			},
			ExpectBody: "Hello, Python Runner of APISIX",
		}),
	)
})
